...
Ns = 20 # number of sample points in each region
...
if __name__ == "__main__":
    ...
    x1 = np.linspace(0, 1, wD.shape[0])
    x2 = np.linspace(0, 1, wD.shape[1])
    x2, x1 = np.meshgrid(x2, x1)
    
    print("Dirichlet eigen value: ", lamD)
    
    plt.figure()
    plt.pcolor(x1, x2, wD, vmin=0, cmap='rainbow')
    plt.colorbar()
    plt.title("landscape (Dirichlet)")
    plt.show()
    
    plt.figure()
    for ind in range(4):
        plt.subplot(2, 2, ind+1)
        plt.pcolor(x1, x2, uD[ind], vmin=-0.5, vmax=1, cmap='rainbow')
        plt.title("eigen mode"+str(ind+1))
        plt.xticks(None)
        plt.yticks(None)
        plt.colorbar()
    plt.show()
    
    print("Neumman eigen value: ", lamD)
    
    plt.figure()
    plt.pcolor(x1, x2, wN, vmin=0, cmap='rainbow')
    plt.colorbar()
    plt.title("landscape (Neumman)")
    plt.show()
    
    plt.figure()
    for ind in range(4):
        plt.subplot(2, 2, ind+1)
        plt.pcolor(x1, x2, uN[ind], vmin=-0.5, vmax=1, cmap='rainbow')
        plt.title("eigen mode"+str(ind+1))
        plt.xticks(None)
        plt.yticks(None)
        plt.colorbar()
