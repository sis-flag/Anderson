...
一维情况下，分别在不同边界条件下模拟问题，得到图\ref{bd-1d}。图中Robin边界条件中取$h=1, \beta=30$，$\alpha$直接取为landscape的最大值的倒数。Neumann边界条件中取$\beta=0$。图中黑色的线代表landscape，彩色的线表示不同特征值下的$u_k(x) / (\lambda_k + \alpha + \beta)$。
\begin{figure}[htbp]
\centering
